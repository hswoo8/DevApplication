...
import com.example.myapplication.util.GraphicUtils;
...
public class PinchImageView extends AppCompatImageView {
    ...
    private static final float DOUBLE_TAP_SCALE = 2.f;

    private static final float MAX_GIF_SCALE = 1.25f;


    private static final float MAX_SCALE_BY_PINCH = MAX_SCALE * 2;
    private static final float MIX_SCALE_BY_PINCH = 0.4f;
    ...
    /**
     * pinch 동작을 비활성화 시키는 flag
     */
    private boolean lockScaleOperation = false;

    /**
     * gif 이면 화면에서 가로 240을 기준으로 작게 표시한다.
     */
    ...
    public static final long NO_NOTIFY = -1;
...
//        innerMatrix = getInnerMatrix(innerMatrix);
//        float innerScale = PinchImageView.MathUtils.getMatrixScale(innerMatrix)[0];
//        float outerScale = PinchImageView.MathUtils.getMatrixScale(mOuterMatrix)[0];
//        float currentScale = innerScale * outerScale;
    ...
    public void lockScaleOperation(boolean lock) {
        lockScaleOperation = lock;
    }
    ...
    protected float getMaxScale() {
        //return gif ? MAX_GIF_SCALE : MAX_SCALE;
        return MAX_SCALE * getInnerScale();
    }
    ...
    private float getMaxScaleByPinch() {
        ...
        return MAX_SCALE_BY_PINCH * innerScale;
    }
    ...
    protected float calculateNextScale(float innerScale, float outerScale) {
        float currentScale = innerScale * outerScale;
        //float currentScale = outerScale;
        if (currentScale > innerScale) {
            return innerScale;
        } else {
    ...
    private float getDoubleTapScale() {
        return DOUBLE_TAP_SCALE * getInnerScale();
    }
    ...
    int BOARDER_WIDTH = GraphicUtils.dpToPx(1);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isReady()) {
            Matrix matrix = MathUtils.matrixTake();
            Matrix imgMatrix = getCurrentImageMatrix(matrix);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            pinchOut.onNext(false);
            pinchIn.onNext(false);
            //在矩阵动画过程中不允许启动滚动模式
    ...
    private void saveScaleContext(float x1, float y1, float x2, float y2) {
        ...
        baseDistance = MathUtils.getDistance(x1, y1, x2, y2);
    ...
    private BehaviorSubject<Boolean> pinchOut = BehaviorSubject.createDefault(false);
    private BehaviorSubject<Boolean> pinchIn = BehaviorSubject.createDefault(false);
    ...
    private void scale(PointF scaleCenter, float scaleBase, float distance, PointF lineCenter) {
        ...
        if (scale > getMaxScaleByPinch() || scale < MIX_SCALE_BY_PINCH) {
            return;
        }

        if (distance - baseDistance > 10) {
            pinchOut.onNext(true);
        }
        if (distance - baseDistance < -10) {
            pinchIn.onNext(true);
        }
